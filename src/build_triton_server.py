...
def build_triton_server(config: AppConfig, model, labels):
    ...
    _write_ensemble_config(config_path=Path(ensemble_path / "config.pbtxt"),
                           config=config
                           )
    _write_model_config(config_path=Path(model_path / "config.pbtxt"),
                        template=config_template_path,
                        config=config,
                        )
    _write_preprocess_config(config_path=Path(preprocess_path / "config.pbtxt"),
                             template=config_template_path,
                             config=config)
...
def _write_model_config(config_path: Path, template: Path, config: AppConfig):
    ...
    _write_config(config_path=config_path,
                  template=template,
                  name=name,
                  input_dim=input_dim,
